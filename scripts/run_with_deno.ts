...
// This script uses the deprecated Deno WASI library, the new `@wasmer/sdk` version does not support Deno, yet.
// It only allows to run files located inside the project's root directory
...
// Make sure `out` and `out/cache` directories exist
await Deno.mkdir('out/cache', {recursive: true});
