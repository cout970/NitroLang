...
echo "PWD is $(realpath .)"

function compile() {
    echo "Compiling $input"
    wasmer run --mapdir "/src:$(realpath ./src)" --mapdir "/out:$(realpath ./scripts/run_in_browser/out)"  --mapdir "/:$(realpath .)" "$compiler" -- "$input" -o "$output" --cache-dir "$cache" --target wasm32-js
